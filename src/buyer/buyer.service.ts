...
export class BuyerService {
    ...
    async updateCartProductQuantity(updateCartProductQuantityDto:UpdateCartProductQuantityDto ) {
        ...
        if(!cartProduct) throw new BadRequestError('product not found in cart');
    ...
    async getCart(cartId: string, userId: string) {
        ...
        if(cart.user.toString() !== userId) throw new NotAuthorizedError();
    ...
    async checkout(userId: string, cardToken: string, userEmail: string) {
        ...
        if(cart.products.length === 0) throw new BadRequestError('your cart is empty!');
        ...
        if(!customer_id) throw new BadRequestError('Invalid data');
        ...
        if(!charge) throw new BadRequestError('Invalid data! could not create the charge!')
    ...
    async updateCustomerStripeCard(userId: string, newCardToken: string) {
        const cart = await this.cartService.findOneByUserId(userId);
        if(!cart) throw new BadRequestError('your cart is empty!');
        if(!cart.customer_id) throw new BadRequestError('you\'re not a customer!');
